...
Большая часть пользовательского интерфейса (для авторизации) выполнена,однако изменить цвет иконок почему-то не выходит, также под кнопкой пока что отсутствует полоска-индикатор (?) авторизации

### Задание 2. Логика 

Авторизация реализована и работает корректно. 
На данном этапе авторизация реализована не лучшим способом, тем не менее, ошибок не возникало
